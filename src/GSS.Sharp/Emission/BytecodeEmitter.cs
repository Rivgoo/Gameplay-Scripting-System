...
namespace GSS.Sharp.Emission
{
    public sealed class BytecodeEmitter
    {
        ...
        private readonly Dictionary<Variant, int> _constantsPoolMap = new(new VariantComparer());
        private readonly List<Variant> _constantsPool = new();
        ...
        private void EmitExpression(BoundExpression node)
        {
            switch (node.Kind)
            {
                case BoundNodeKind.LiteralExpression:
                    ...
                    var val = lit.Type == TypeSymbol.Int ? Variant.FromInt((int)lit.Value!) :
                              lit.Type == TypeSymbol.Float ? Variant.FromFloat((float)lit.Value!) :
                              lit.Type == TypeSymbol.Bool ? Variant.FromBool((bool)lit.Value!) : Variant.FromObject(lit.Value);
                ...
                case BoundNodeKind.CallExpression:
                    ...
                    EmitInstruction(new Instruction(OpCode.CallExt, GetOrAddMetadata(call.Method), args.Length));
                    break;
                ...
                case BoundNodeKind.IndexExpression:
                    ...
                    EmitInstruction(new Instruction(OpCode.LdElem, colReg));
                ...
                case BoundNodeKind.IndexAssignmentExpression:
                    ...
                    EmitInstruction(new Instruction(OpCode.StElem, cr, ir));
                    _allocator.Free(ir);
                    _allocator.Free(cr);
                    break;
                ...
                case BoundNodeKind.ConversionExpression:
                    ...
                    if (conv.Type == TypeSymbol.Float && conv.Expression.Type == TypeSymbol.Int)
                        EmitInstruction(new Instruction(OpCode.ConvIntToFloat));
                    else if (conv.Type == TypeSymbol.Int && conv.Expression.Type == TypeSymbol.Float)
                        EmitInstruction(new Instruction(OpCode.ConvFloatToInt));
                    else if (conv.Type == TypeSymbol.String && conv.Expression.Type != TypeSymbol.String)
                        EmitInstruction(new Instruction(OpCode.ConvToStr));
                    break;
                ...
                case BoundNodeKind.UnaryExpression:
                    ...
                    {
                        BoundUnaryOperatorKind.Negation => unary.Type == TypeSymbol.Int ? OpCode.NegInt : OpCode.NegFloat,
                        BoundUnaryOperatorKind.LogicalNegation => OpCode.Not,
                        BoundUnaryOperatorKind.BitwiseNegation => throw new InvalidOperationException("BitwiseNot not mapped to OpCode."),
        ...
        private OpCode GetBinaryOpCode(BoundBinaryOperatorKind kind, TypeSymbol leftType)
        {
            return kind switch
            {
                BoundBinaryOperatorKind.Addition => leftType == TypeSymbol.String ? OpCode.Concat : (leftType == TypeSymbol.Int ? OpCode.AddInt : (leftType == TypeSymbol.Float ? OpCode.AddFloat : OpCode.AddVec)),
                BoundBinaryOperatorKind.Subtraction => leftType == TypeSymbol.Int ? OpCode.SubInt : (leftType == TypeSymbol.Float ? OpCode.SubFloat : OpCode.SubVec),
                BoundBinaryOperatorKind.Multiplication => leftType == TypeSymbol.Int ? OpCode.MulInt : (leftType == TypeSymbol.Float ? OpCode.MulFloat : OpCode.MulVec),
                ...
                BoundBinaryOperatorKind.Equals => OpCode.Eq,
                BoundBinaryOperatorKind.NotEquals => OpCode.Neq,
                BoundBinaryOperatorKind.Less => OpCode.Lt,
                BoundBinaryOperatorKind.LessOrEquals => OpCode.Lte,
                BoundBinaryOperatorKind.Greater => OpCode.Gt,
                BoundBinaryOperatorKind.GreaterOrEquals => OpCode.Gte,
